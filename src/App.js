...
class App extends Component {
  state = {
    randomImage: imagesList[Math.ceil(Math.random() * 30 - 1)],
    listData: [],
    tabSelected: 'FRUIT',
    timer: 60,
    score: 0,
  ...
  componentWillUnmount() {
    ...
    if (timer === 0) {
      ...
      this.setState({didNotMatch: true})
    }
  }
  ...
  onTabClicked = event => {
    this.setState({tabSelected: event.target.id}, this.getList)
  }

  thumbnailSelected = event => {
    const {randomImage} = this.state
    console.log(event.target.id === randomImage.id)
    if (event.target.id === randomImage.id) {
  ...
  playAgain = () => {
    this.setState({didNotMatch: false, timer: 60, score: 0})
  }

  render() {
    const {randomImage, listData, timer, score, didNotMatch} = this.state
    return (
      <div className="bg">
        <ul className="header">
          <li>
